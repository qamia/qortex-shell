...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Qortex', separator: { label: ' - ' } }), 'Qortex');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Qortex', separator: { label: ' - ' } }), 'Untitled-1 - Qortex');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Qortex', separator: { label: ' - ' } }), 'monaco - Qortex');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Qortex', separator: { label: ' - ' } }), 'somefile.txt - monaco - Qortex');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Qortex', separator: { label: ' - ' } }), '* somefile.txt - monaco - Qortex');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Qortex', separator: { label: ' - ' } }), 'monaco - Qortex');
	});
