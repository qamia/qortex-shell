	...
	private filterServers(): void {
		...
		if (extensionServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('extensionGroupDescription', "MCP servers contributed by installed Qortex extensions."),
				collapsed,
			});
			if (!collapsed) {
		...
		if (otherBuiltinServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('builtInGroupDescription', "MCP servers built into Qortex. These are available automatically."),
				collapsed,
			});
			if (!collapsed) {
